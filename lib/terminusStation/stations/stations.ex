defmodule TerminusStation.Stations do
  ...
  defp format_station(station, newline) do
    ...
    station.name <>
      ...
      newline <>
      legend <>
      newline <> List.to_string(for route <- station.routes, do: format_route(route, newline))
  end
    ...
    alerts =
      Repo.all(
        from(
          ...
          on:
            schedule.trip_id == alert.trip_id or schedule.stop_id == alert.stop_id or
              schedule.trip_id == alert.trip_id,
  ...
  def get_by_station(station) do
    ...
    for station <- stations do
      ...
      prediction_track_code =
        if station.prediction_track_code == nil, do: "TBD", else: station.prediction_track_code

      status = if station.status == nil, do: "no data", else: station.status

      departure =
        station.departure |> Timex.Timezone.convert("America/New_York")
        |> Timex.format!("%H:%M", :strftime)

      prediction_departure =
        station.departure |> Timex.Timezone.convert("America/New_York")
        |> Timex.format!("%H:%M", :strftime)
  ...
  def get_board(newline) do
    northText =
      format_station(%{name: "North Station", routes: get_by_station("place-north")}, newline)

    southText =
      format_station(%{name: "South Station", routes: get_by_station("place-sstat")}, newline)
  ...
  def remove(_data, type, id) do
    ...
    case Repo.get_by(table, %{column => id}) do
      ...
      # don't remove because it seems my predictions are getting deleted when i still want them
      record ->
        Repo.delete(record)
    end
  end
  ...
  def update(data, _from, "schedule", id) do
    record =
      case from(r in Schedule) |> where([r], r.schedule_id == ^id) |> Repo.one() do
        nil -> %Schedule{}
        record -> record
      end

    departureTime =
      data |> map_no_nil("attributes")
      |> Map.get("departure_time", Map.get(record, :departure_time, ""))

    record
    |> Schedule.changeset(%{
      schedule_id: id,
      trip_id:
        data |> map_no_nil("relationships") |> map_no_nil("trip") |> map_no_nil("data")
        |> Map.get("id", Map.get(record, :trip_id, nil)),
      stop_id:
        data |> map_no_nil("relationships") |> map_no_nil("stop") |> map_no_nil("data")
        |> Map.get("id", Map.get(record, :stop_id, nil)),
      route_id:
        data |> map_no_nil("relationships") |> map_no_nil("route") |> map_no_nil("data")
        |> Map.get("id", Map.get(record, :route_id, nil)),
      prediction_id:
        data |> map_no_nil("relationships") |> map_no_nil("prediction") |> map_no_nil("data")
        |> Map.get("id", Map.get(record, :prediction_id, nil)),
      pickup_type:
        data |> map_no_nil("attributes")
        |> Map.get("pickup_type", Map.get(record, :pickup_type, nil)),
  ...
  def update(data, _from, "trip", id) do
    record =
      case from(r in Trip) |> where([r], r.trip_id == ^id) |> Repo.one() do
        nil -> %Trip{}
        record -> record
      end
    ...
    |> Trip.changeset(%{
      trip_id: id,
      vehicle_id:
        data |> map_no_nil("relationships") |> map_no_nil("vehicle") |> map_no_nil("data")
        |> Map.get("id", Map.get(record, :vehicle_id, nil)),
      route_id:
        data |> map_no_nil("relationships") |> map_no_nil("route") |> map_no_nil("data")
        |> Map.get("id", Map.get(record, :route_id, nil)),
      block_id:
        data |> map_no_nil("attributes") |> Map.get("block_id", Map.get(record, :block_id, nil)),
      destination:
        data |> map_no_nil("attributes")
        |> Map.get("headsign", Map.get(record, :destination, nil))
    })
    |> Repo.insert_or_update()
  end
  ...
  def update(data, _from, "route", id) do
    record =
      case from(r in Route) |> where([r], r.route_id == ^id) |> Repo.one() do
        nil -> %Route{}
        record -> record
      end
    ...
    |> Route.changeset(%{
      route_id: id,
      long_name:
        data |> map_no_nil("attributes") |> Map.get("long_name", Map.get(record, :long_name, nil)),
      short_name:
        data |> map_no_nil("attributes")
        |> Map.get("short_name", Map.get(record, :short_name, nil)),
  ...
  def update(data, _from, "stop", id) do
    record =
      case from(r in Stop) |> where([r], r.stop_id == ^id) |> Repo.one() do
        nil -> %Stop{}
        record -> record
      end
    ...
    |> Stop.changeset(%{
      ...
      platform_name:
        data |> map_no_nil("attributes")
        |> Map.get("platform_name", Map.get(record, :platform_name, nil)),
      platform_code:
        data |> map_no_nil("attributes")
        |> Map.get("platform_code", Map.get(record, :platform_code, nil)),
      parent:
        data |> map_no_nil("relationships") |> map_no_nil("parent_station") |> map_no_nil("data")
        |> Map.get("id", Map.get(record, :parent, nil))
    })
    |> Repo.insert_or_update()
  end
  ...
  def update(data, _from, "vehicle", id) do
    record =
      case from(r in Vehicle) |> where([r], r.vehicle_id == ^id) |> Repo.one() do
        nil -> %Vehicle{}
        record -> record
      end
  ...
  def update(data, from, "prediction", id) do
    record =
      case from(r in Prediction) |> where([r], r.prediction_id == ^id) |> Repo.one() do
        nil -> %Prediction{}
        record -> record
      end
    ...
    |> Prediction.changeset(%{
      ...
      schedule_relationship:
        data |> map_no_nil("attributes")
        |> Map.get("schedule_relationship", Map.get(record, :schedule_relationship, nil)),
      departure_time: departureTime
    })
    |> Repo.insert_or_update()
  end
  ...
  def update(data, _from, "alert", id) do
    record =
      case from(r in Alert) |> where([r], r.alert_id == ^id) |> Repo.one() do
        nil -> %Alert{}
        record -> record
      end
    ...
    |> Alert.changeset(%{
      alert_id: id,
      route_id:
        data |> map_no_nil("attributes") |> map_no_nil("informed_entity") |> List.first()
        |> Map.get("route", Map.get(record, :route_id, nil)),
      trip_id:
        data |> map_no_nil("attributes") |> map_no_nil("informed_entity") |> List.first()
        |> Map.get("trip", Map.get(record, :trip_id, nil)),
      stop_id:
        data |> map_no_nil("attributes") |> map_no_nil("informed_entity") |> List.first()
        |> Map.get("stop", Map.get(record, :stop_id, nil)),
